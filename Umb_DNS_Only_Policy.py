...
def dns_policy(dns_policy_name,dns_policy_des,match_vpn):
    ...
    response_global = requests.post(f'https://vManage_IP:vManage_Port/dataservice/template/policy/definition/dnssecurity',
...
def dns_local_policy(dns_policy_name,dns_policy_des,dns_policy):
    ...
    print()
    response_global = requests.post(f'https://vManage_IP:vManage_Port/dataservice/template/policy/security/',
                                    headers=header, verify=False, json=(payload))
    if response_global.status_code !=200:
