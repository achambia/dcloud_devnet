...
def bind_sig_to_device_temp(temp_name,sig_tunnel_name,sig_tunnel_des,ipsec_tunnel_primary_num,ipsec_tunnel_secondary_num,pri_source_int,sec_source_int,tracker_ip,umb_key,umb_secret,org,sig_cred_name,sig_cred_des):
    ...
    response_global_sig_creds = requests.post(f'https://vManage_IP:vManage_Port/dataservice/template/feature/',
    ...
    for y in devic_id:
        ...
        for z in device_temp['generalTemplates']:
            if z['templateType'] == "cisco_vpn":
                response_local = requests.get(f'https://vManage_IP:vManage_Port/dataservice/template/feature/object/{z["templateId"]}',
                                              headers=header,
                                              verify=False,)
        ...
        response_global_update = requests.put(f'https://vManage_IP:vManage_Port/dataservice/template/device/{y}', headers=header,
