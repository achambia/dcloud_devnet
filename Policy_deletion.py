...
def policy_delete(policy_name,dns_policy):
    response_global = requests.get(f'https://vManage_IP:vManage_Port/dataservice/template/policy/security/',
                                    headers=header, verify=False)
    resp_dns_sec = requests.get(f'https://vManage_IP:vManage_Port/dataservice/template/policy/definition/dnssecurity',
                                    headers=header, verify=False)
        ...
        if x['policyName'] == policy_name:
            if x['mastersAttached'] == 0:
                response_global_delete = requests.delete(
                    f'https://vManage_IP:vManage_Port/dataservice/template/policy/security/{x["policyId"]}',
                    headers=header, verify=False)
    ...
    for y in resp_dns_sec.json()['data']:
        if y['name'] == dns_policy:
            resp_dns = requests.delete(
                f'https://vManage_IP:vManage_Port/dataservice/template/policy/definition/dnssecurity/{y["definitionId"]}',
                headers=header, verify=False)
