...
def attach_policy_to_cedge(policy_name,temp_name):
    ...
    push_verification = ''
    local_policy = requests.get(f'https://198.18.133.200:8443/dataservice/template/policy/security/',headers=header,
                                              verify=False,)
    ...
    for x in devic_id:
        ...
        response_global_device = requests.get(f'https://198.18.133.200:8443/dataservice/template/device/object/{x}',
                                              headers=header,
                                              verify=False,)
        ...
        response_global = requests.put(f'https://198.18.133.200:8443/dataservice/template/device/{x}',headers=header, verify=False,json=device_temp)
        ...
        response_global_input = requests.post(f'https://198.18.133.200:8443/dataservice/template/device/config/input/',headers=header, verify=False,json=input_payload)
